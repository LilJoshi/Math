...
function_w = lambda t: lambda n: lambda: t(n)
...
def repeat(f, n=1):
	yield n
	yield from repeat(f, f(n))

intgen = lambda n: repeat(identity, n)
...
evens = multiples(2, 2) #or, applyf(naturals, lambda x:2*x)
odds = multiples(2, 1)

triangles = accumulatef(naturals, 0)
squares = accumulatef(odds, 0) #or, applyf(naturals, double)
polygons = lambda n: accumulatef(multiples(n-2, 1), 0)

powers = lambda n: repeatf(times(n))
...
# For testing purposes
test = lambda genf: genftest(genf, 0)()
def alltests():
	genf_list = [naturals, evens, odds, triangles, squares]
